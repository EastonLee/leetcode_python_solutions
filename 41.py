...
class Solution:
    def largeGroupPositions(self, S):
        ...
        :rtype: List[List[int]]
        """
        S = [list(g) for k, g in itertools.groupby(S)]
        rst = [[0,len(S[0]) - 1]]
        for s in S[1:]:
            rst.append([rst[-1][1] + 1, rst[-1][1] + len(s)])
        return [rst[i] for i, s in enumerate(S) if len(s) >= 3]
...
class Test(unittest.TestCase):
    ...
    def test(self):
        ...
        for ci, co in cases:
            assert Solution().largeGroupPositions(ci) == co
